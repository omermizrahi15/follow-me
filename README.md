...
| **CI · services** | [`ci-services.yml`](.github/workflows/ci-services.yml) | PR touching `supabase/functions/**` | per-changed-function Deno lint + typecheck + test, one matrix job each; when no function changed it still lints and tests `_shared/` |
