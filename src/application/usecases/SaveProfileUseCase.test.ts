...
function makeProfile(overrides: Partial<{ displayName: string; avatarUrl: string | null }> = {}): PublisherProfile {
...
describe('SaveProfileUseCase / LoadProfileUseCase', () => {
  it('persists and loads a profile', async (): Promise<void> => {
    ...
    await new SaveProfileUseCase(repo).execute(makeProfile({ avatarUrl: 'https://cdn.test/a.jpg' }));
  ...
  it('overwrites an existing profile for the same publisher', async (): Promise<void> => {
    ...
    await save.execute(makeProfile({ displayName: 'Omer M', avatarUrl: 'https://cdn.test/a.jpg' }));
  ...
  // Onboarding makes the photo optional (and "Skip for now" saves nothing
  // extra), so a name-only profile must round-trip cleanly.
  it('saves a name-only profile, reading the avatar back as null', async (): Promise<void> => {
    const repo = new InMemoryPublisherProfileRepository();
  ...
  it('normalizes an empty / whitespace-only avatar to null on save', async (): Promise<void> => {
    const repo = new InMemoryPublisherProfileRepository();
    await new SaveProfileUseCase(repo).execute(makeProfile({ avatarUrl: '   ' }));
  ...
  it('clears a previously set avatar when re-saved empty', async (): Promise<void> => {
    const repo = new InMemoryPublisherProfileRepository();
    const save = new SaveProfileUseCase(repo);
    await save.execute(makeProfile({ avatarUrl: 'https://cdn.test/a.jpg' }));
    await save.execute(makeProfile({ avatarUrl: null }));
