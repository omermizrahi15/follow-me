...
describe('SharePhotoUseCase', () => {
  it('saves the photo and returns a dto', async () => {
  ...
  it('does not notify revoked subscribers', async () => {
    const { useCase, subscriberRepo, notifier } = makeSut();
    const revoked = Subscriber.create({
      id: 'sub-revoked',
      publisherId: 'user-1',
      contactHandle: '+972509999999',
      status: 'revoked',
    });
  ...
  it('does not notify subscribers of other publishers', async () => {
    const { useCase, subscriberRepo, notifier } = makeSut();
