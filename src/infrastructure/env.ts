 ...
 * supabase/client.ts and container.ts continues against inert placeholders, and
 * the UI renders the setup message instead of the navigator. Nothing reaches
 * the network in that state, because no screen that talks to a use case is
 * ever mounted.
 *
 * This lives in infrastructure rather than composition because the shared
 * Supabase client (issue #115) is the earliest thing to need a value, and
 * infrastructure cannot import from the composition root. The UI reads
 * `envSetupMessage` back through composition/env.ts, which is the boundary it
 * is allowed to cross.
 *
