// Pure batch-location helpers for the autonomous auto-post job (issue #23).
// Deno mirror of src/domain/services/postingLocation.ts — the functions dir is a
// separate Deno project and can't import the React-Native app source, so the
// pure logic is duplicated here and covered by its own tests.
...
/**
 * The coordinate that stands for a group of photos: the per-axis median, so a
 * single outlier (a screenshot with stale GPS) doesn't drag the place away from
 * where most of the group was shot.
 */
...
/**
 * Representative coordinates for the batch's major places, largest group first,
 * capped at `max`. Photos are greedily clustered — a photo joins the first
 * cluster whose representative is within CLUSTER_RADIUS_KM, else starts a new
 * one — so a trip covering two cities yields both, while same-city batches
 * collapse to a single median coordinate.
 */
